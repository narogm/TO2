...
public class Grade {
    ...
    public static boolean gradeStudent(final Student student, final Course course, final float grade) {
    	String gradeStudentSql = String.format("INSERT INTO grade (grade, student_id, course_id) VALUES ('%f', %d, %d)", grade, student.id(), course.id());

        try {
            int id = QueryExecutor.createAndObtainId(gradeStudentSql);
            if(id > -1)
                return true;
